...
class featuredproperty extends WP_Widget 
{
	...
	public function form($instance)
	{
		...
		echo "</p>";
		echo "<p>";
		echo "<label for='".$this->get_field_id('house')."'>" . __('Home Square Footage') . ":</label> " ;
		...
		echo "</p>";
		echo "<p>";
		echo "<label for='".$this->get_field_id('lot')."'>" . __('Lot Square Footage') . ":</label> " ;
		echo "<input type='text' class='widefat' id='".$this->get_field_id('lot')."' name='".$this->get_field_name('lot')."' value='" . esc_attr($instance['lot'] ) . "' />" ;
		echo "</p>";
		echo "<p>";
		echo "<label for='".$this->get_field_id('desc')."'>" . __('Description') . ":</label> " ;
	...
	public function widget($args, $instance) 
	{
		...
		echo '<div class="row"><div class="col-md-6 fprop-attrb-title">Type:</div>';
		echo '<div class="col-md-6 fprop-attrb">'.$instance['type'].'</div></div>';
		}
		
		if ( !empty( $instance['beds'] ) ) {
		echo '<div class="row"><div class="col-md-6 fprop-attrb-title">Beds:</div>';
		echo '<div class="col-md-6 fprop-attrb">'.$instance['beds'].'</div></div>';
		}
		
		if ( !empty( $instance['baths'] ) ) {
		echo '<div class="row"><div class="col-md-6 fprop-attrb-title">Baths:</div>';
		echo '<div class="col-md-6 fprop-attrb">'.$instance['baths'].'</div></div>';
		}
		
		if ( !empty( $instance['house'] ) ) {
		echo '<div class="row"><div class="col-md-6 fprop-attrb-title">Home Size:</div>';
		echo '<div class="col-md-6 fprop-attrb">'.$instance['house'].'</div></div>';
		}
		
		if ( !empty( $instance['lot'] ) ) {
		echo '<div class="row"><div class="col-md-6 fprop-attrb-title">Lot Size:</div>';
		echo '<div class="col-md-6 fprop-attrb">'.$instance['lot'].'</div></div>';
		}
		
		if ( !empty( $instance['desc'] ) ) {
		echo '<div class="row"><div class="col-md-12 fprop-attrb-title">Description:</div>';
